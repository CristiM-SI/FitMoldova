...
const Activitati: React.FC = () => {
    ...
    const { activitatiCurente, addActivitate, removeActivitate, joinActivity, leaveActivity, joinedActivities, joinedActivityIds } = useDashboardData();
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            const created: ActivityDto = {
                id: createdId, name: payload.name, type: payload.type,
                distance: payload.distance, duration: payload.duration,
                calories: payload.calories, date: payload.date,
                description: payload.description, imageUrl: payload.imageUrl,
                createdBy: user?.id?.toString() || '', participantsCount: 0,
            };
    ...
    // ── Exclude activitatile deja joined sau adaugate local ───────────────────
    const addedIds = useMemo(
        () => new Set([...activitatiCurente.map(a => a.id), ...joinedActivityIds]),
        [activitatiCurente, joinedActivityIds]
    );

    const recommendations = useMemo(
        () => apiActivities.filter(a => !addedIds.has(a.id)),
        [apiActivities, addedIds]
    );

    const adaugaActivitate = async (act: ActivityDto) => {
        try {
            await joinActivity(act.id);
        } catch {
            // daca deja inscris, ignoram eroarea
        }
        addActivitate({
            id: act.id, name: act.name, type: act.type,
            distance: act.distance, duration: act.duration,
            calories: act.calories, date: act.date,
        });
        completeFirstActivity();
    };

    // ── Combina locale + DB (fara duplicate) ─────────────────────────────────
    ...
    const { totalCalories, totalDist } = useMemo(() => ({
        totalCalories: toateActivitatile.reduce((s, a) => s + a.calories, 0),
        totalDist: toateActivitatile.reduce((s, a) => s + (parseFloat(a.distance) || 0), 0),
    }), [toateActivitatile]);
    ...
    return (
        <DashboardLayout>
            ...
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mb: 3 }}>
                {[
                    { label: 'Activitati Inregistrate', value: toateActivitatile.length, Icon: DirectionsRun, color: '#3b82f6', bg: '#eff6ff' },
                    ...
                    <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{ px: 2 }}>
                        <Tab label={
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                Activitatile Mele
                                <Chip label={toateActivitatile.length} size="small"
                            ...
                            {toateActivitatile.length === 0 ? (
                                ...
                                <>
                                        ...
                                        {toateActivitatile.map((act) => (
                                            <Paper key={act.id} elevation={0} sx={{
                                                ...
                                                <IconButton size="small" onClick={async () => {
                                                    try { await leaveActivity(act.id); } catch { /* ignore */ }
                                                    removeActivitate(act.id);
                                                }}
                                    ...
                                    <Box sx={{ mt: 2.5, pt: 2.5, borderTop: '1px solid #f0f4f8', display: 'flex', gap: 0 }}>
                                        {[
                                            { label: 'Total activitati', value: toateActivitatile.length },
                ...
                <Card elevation={0} sx={{ borderRadius: 3, border: '1px solid #e8edf3', mt: 3 }}>
                    <CardContent sx={{ p: 3 }}>
                        ...
                        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                            {Object.entries(
                                toateActivitatile.reduce<Record<string, number>>((acc, a) => {
                                    acc[a.type] = (acc[a.type] || 0) + 1;
                                    return acc;
                                }, {})
                            ).map(([type, count]) => {
                                const pct = Math.round((count / toateActivitatile.length) * 100);
